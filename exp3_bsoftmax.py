...
def evaluate(model, loader, num_classes, spc):
    ...
    with torch.no_grad():
        ...
        for (imgs, labels, dicoms) in loader:
            ...
            loss = balanced_softmax_loss(labels, output, spc)
...
def balanced_softmax_loss(labels, logits, samples_per_cls):   
    
    spc = torch.tensor(samples_per_cls).float().to(device)
    spc = spc.unsqueeze(0).repeat(labels.size(0), 1)
    output = logits + spc.log()
    ...
    for epoch in range(epochs):
        ...
        for (imgs, labels, dicom_id) in train_loader:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                output = encoder(imgs)
                # loss = criterion(output, labels)
                loss = balanced_softmax_loss(labels, output, train_spc)
