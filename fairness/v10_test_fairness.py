    ...
    with torch.no_grad():
        ...
        for (imgs, labels, dicoms, ages, genders, races) in data_loader:
            ...
            for label, output, gender in zip(labels, outputs, genders):
                if gender == 'F':
                    record['gender'][0]['target'] = torch.cat((record['gender'][0]['target'], label.reshape(1, -1)), 0)
                    record['gender'][0]['predict'] = torch.cat((record['gender'][0]['predict'], output.reshape(1, -1)), 0)
                elif gender == 'M':
                    record['gender'][1]['target'] = torch.cat((record['gender'][1]['target'], label.reshape(1, -1)), 0)
                    record['gender'][1]['predict'] = torch.cat((record['gender'][1]['predict'], output.reshape(1, -1)), 0)
                else:                    
                    continue
...
if __name__ == "__main__":
    ...
    folder = "PATH/hc3_extra"
    output_path = f"{folder}/fairness/results/0609_v10"
    ...
    # model_names = ['LDAMLoss', 'CBLoss', 'cRT', 'LWS', 'MLROS', 'ROS', 'simCLR', 'WBCELoss']
    model_names = ['bsoftmaxLoss']
    
    for i, model_name in enumerate(model_names):
        records = {'val': {}, 'test': {}}
        if model_name == 'LDAMLoss':      
            checkpoint = torch.load(f"{folder}/weights/LDAMLoss/model_best.pt")
            encoder = transformer_model().to(device)
        elif model_name == 'CBLoss':        
            checkpoint = torch.load(f"{folder}/weights/CBLoss/model_best.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'cRT':        
            checkpoint = torch.load(f"{folder}/weights/cRT/model_best_finetuned.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'focalLoss':        
            checkpoint = torch.load(f"{folder}/weights/focalLoss/model_best.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'LWS':        
            checkpoint = torch.load(f"{folder}/weights/LWS/model_best_finetuned.pt")     
            encoder = transformer_model_LWS().to(device)
        elif model_name == 'MLROS':        
            checkpoint = torch.load(f"{folder}/weights/MLROS/model_best.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'ROS':        
            checkpoint = torch.load(f"{folder}/weights/ROS/model_best.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'simCLR':        
            checkpoint = torch.load(f"{folder}/weights/simCLR/model_best.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'WBCELoss':        
            checkpoint = torch.load(f"{folder}/weights/WBCELoss/model_best.pt")     
            encoder = transformer_model().to(device)
        elif model_name == 'bsoftmaxLoss':        
            checkpoint = torch.load(f"{folder}/weights/bsoftmaxLoss/model_best.pt")     
            encoder = transformer_model().to(device)
        ...
        with open(f'{output_path}/{model_name}_{args.dataSeed}_record.pkl', 'wb') as file:
