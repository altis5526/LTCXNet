...
class FocalLoss(nn.Module):
    def __init__(self, alpha=1.0, gamma=2.0):
        ...
        Args:
            alpha (float or tensor): Weighting factor for the positive class (helps with imbalance).
            gamma (float): Focusing parameter (higher gamma means more focus on hard examples).
            reduction (str): 'mean', 'sum', or 'none' for how the loss should be reduced.
        """
    ...
    def forward(self, logits, targets):
        """
        Args:
            logits: Raw predictions (before sigmoid) of shape (batch_size, num_classes).
            targets: Binary labels of shape (batch_size, num_classes), with values 0 or 1.
        ...
        probs = torch.clamp(probs, min=1e-6, max=1 - 1e-6)  # Prevent numerical instability

        # Compute BCE loss per class
        bce_loss = -targets * torch.log(probs) - (1 - targets) * torch.log(1 - probs)

        # Compute focal weight
        focal_weight = (1 - probs) ** self.gamma * targets + probs ** self.gamma * (1 - targets)

        # Apply alpha (if per-class weights are given)
        if isinstance(self.alpha, (float, int)):
            focal_weight *= self.alpha
        elif isinstance(self.alpha, torch.Tensor):
            focal_weight *= self.alpha.unsqueeze(0)  # Broadcast over batch
