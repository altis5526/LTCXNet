...
def evaluate(model, loader, num_classes, spc, loss_type):
    model.eval()
    running_loss = 0.0
    total = 0
    ...
    with torch.no_grad():
        ...
        for (imgs, labels, dicoms) in loader:
            ...
            loss = CB_loss(labels, output, spc, num_classes, 0.9999, loss_type)
...
def CB_loss(labels, logits, samples_per_cls, no_of_classes, beta, type):
    
    effective_num = 1.0 - np.power(beta, samples_per_cls)
    weights = (1.0 - beta) / np.array(effective_num)
    weights = weights / np.sum(weights) * no_of_classes     # [n_cls,] 
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for (imgs, labels, dicom_id) in train_loader:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                output = encoder(imgs)
                # loss = criterion(output, labels)
                loss = CB_loss(labels, output, train_spc, num_classes, 0.9999, args.loss_type)
        ...
        total = count
        mAP, mAPs, val_running_loss, val_total = evaluate(encoder, val_loader, num_classes, val_spc, args.loss_type)
    ...
    mAP, mAPs, test_running_loss, test_total = evaluate(encoder, test_loader, num_classes, test_spc, args.loss_type)
